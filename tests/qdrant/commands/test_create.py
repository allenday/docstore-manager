...
def test_create_collection_invalid_distance(mock_client):
    ...
    # Check exact message if possible, or key parts
    assert "Invalid distance metric specified: 'INVALID_DISTANCE'" in str(exc_info.value)
    mock_client.create_collection.assert_not_called()
    mock_client.recreate_collection.assert_not_called()
...
def test_create_collection_already_exists_no_overwrite(mock_client, caplog):
    ...
    mock_client.create_collection.side_effect = UnexpectedResponse( # Use correct exception class
